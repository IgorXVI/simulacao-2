<script>
	...
	const calcTable = () => {
		...
		const getRandomEL = (arr = [0]) =>
			arr[getRandBetween(0, arr.length - 1)] || 0;
		...
		while (true) {
			...
			const tempoDeChegadaNoRelogio =
				lastEl.tempoDeChegadaNoRelogio + tempoDesdeAUltimaChegada;
			...
			const tempoDoClienteNaFila =
				lastEl.tempoFinalDoServicoNoRelogio >= tempoDeChegadaNoRelogio
					? lastEl.tempoFinalDoServicoNoRelogio -
					  tempoDeChegadaNoRelogio
					: 0;

			const tempoDeInicioDeServicoNoRelogio =
				tempoDeChegadaNoRelogio + tempoDoClienteNaFila;

			const tempoFinalDoServicoNoRelogio =
				tempoDeInicioDeServicoNoRelogio + tempoDoServico;

			const tempoDoClienteNoSistema =
				tempoDoServico + tempoDoClienteNaFila;
		...
		const sumTableRows = (attr = "") =>
			newTable.reduce((acc, el) => acc + el[attr], 0);

		const quantidadeDeClientes = newTable.length;

		const numClienteEsperaram = newTable.filter(
			(el) => el.tempoDoClienteNaFila > 0
		).length;
		...
		const tempoMedioEspera = (
			sumsRow.tempoDoClienteNaFila / quantidadeDeClientes
		).toFixed(2);

		const probClienteFila = (
			100 *
			(numClienteEsperaram / quantidadeDeClientes)
		).toFixed(2);

		const probOperadorLivre = (
			100 *
			(sumsRow.tempoLivreDoOperador / tempoSimulacao)
		).toFixed(2);

		const tempoMedioServico = (
			sumsRow.tempoDoServico / quantidadeDeClientes
		).toFixed(2);

		const tempoMedioSistema = (
			sumsRow.tempoDoClienteNoSistema / quantidadeDeClientes
		).toFixed(2);
...
<div>
	<button on:click={calcTable}>Simular</button>
	<button on:click={clearAll}>Limpar</button>
</div>

<div>
	<label for="tempoSimulacao">Tempo da Simulação (em minutos)</label>
	<input type="number" bind:value={tempoSimulacao} id="tempoSimulacao" />
</div>
...
<table>
	<thead>
		...
		<th>Tempo live do operador (minutos)</th>
	</thead>
	<tbody>
